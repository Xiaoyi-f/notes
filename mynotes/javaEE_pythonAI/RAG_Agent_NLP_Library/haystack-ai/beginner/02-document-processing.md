...
# 创建检索器 BM25 -> 分词 + 计算相关性分数 + 排序返回top_k
...
def buildPrompt(question):
    ...
    retrievedDocs = retrieverResult["documents"] # 获取检索到的文档
    promptResult = prompt.run(documents=retrievedDocs, query=question) # 将检索到的文档和问题注入提示词
