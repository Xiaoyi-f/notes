 ...
 * abstract 抽象态 --> 抽象方法 --> 抽象类 --> 抽象类不可以实例化 
 */
 ...
 * 
 * 多态转型:
 * 向上(大)转型 --> 父类 对象 = new 子类();
 * 向下(小)转型 --> 子类 对象 = (子类)父类引用;
 * 
 * 向上转型成员调用规则:
 * 1)可以调用父类中所有成员(遵循修饰符规则)
 * 2)不能调用子类特有成员
 * 3)因为在编译阶段,能调用哪些成员是由编译类型来决定的 --> 左边为编译类型,右边为实际类型,编译器只按照左边进行语法检查
 * 
 * 向下转型成员调用规则:
 * 1)可以调用子类中特有成员和继承父类成员(遵循修饰符规则和继承规则)
 * 2)向下转型只能用于回转实际类型 使用 (引用 instanceof 类型) 进行提前判别 否则运行阶段会报错
 *  
 * 抽象: 在java中没有方法体的方法应该定义为抽象方法, 而类中如果存在抽象方法则该类必须定义为抽象类
 * 实体类继承抽象类必须重写抽象方法,抽象类继承抽象类继续抽象定义即可
 * 抽象类中可以写实例方法
 * 
...
/**
 * 补充:
 * 泛型 --> 泛型类 泛型方法 泛型接口 编译时候提前类型检查 --> 确保类型安全
 * 使用 --> 泛型参数化 <T>泛型变量 T泛型参数调用
 * 泛型类 --> 类名后参数化
 * 泛型方法 --> 修饰符后参数化
 * 泛型接口 --> 接口名后参数化 
 * 
 * 通配符:
 * "界 --> 取自数学概念"
 * 无界 --> <?> 表示任意类型 --> 只用于读 不可改动内部数据 
 * 上界 --> <? extends T> 表示T类或T的子类 --> 可删读 不可写 
 * 下界 --> <? super T> 表示T类或T的父类 --> 可删读 可写  
 */
